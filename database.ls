...
add-index-for-collections = !(callback)->
  db.createIndex 'locations', 'urls', {unique: true}, callback
...
query-collection = !(collection-name, query-obj, callback)->
  (db) <-! get-db
  # create a index for chats 
  #db.at-plus['chats'].ensure-index {"joins.userId": 1}, {unique: 1}
...
save-single-document-in-database = !(collection-name, single-doc, callback)->
  ...
  if err and err.code is 11000 then callback err # which means duplication
